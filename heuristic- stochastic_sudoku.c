...
#include<stdlib.h>
#include<string.h>
short matrix[81],indices[50],count=0,i,j,best,access[9][9][10];
void make_access();
void empty();
void print_access();
short get_place();
...
void make_access()
{
	short place=1,k,permut[9]={0},min;
	for(i=0;i<9;++i)
	{
		for(j=0;j<9;++j)
		{
	...
	puts("The permutations are as under:\n");
	for(i=0;i<9;++i)
		printf("%2d\n",permut[i]);
	min=permut[0];
	for(i=0;i<9;++i)
